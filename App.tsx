...
import { PRESET_IMAGES, EDGE_CASE_EXAMPLES, INITIAL_TRANSFORMATIONS, DIFFICULTY_PRESETS } from './constants';
    ...
    <div className={`min-h-screen flex flex-col items-center transition-colors duration-300 ${themeClasses.bg}`}>
      ...
      <nav className={`w-full sticky top-0 z-50 backdrop-blur-md border-b px-6 py-3 flex justify-between items-center transition-colors ${themeClasses.nav}`}>
        ...
        <div className="md:hidden">
          <MenuIcon size={24} className={themeClasses.headerText} />
        </div>
      </nav>
      ...
      <div className="w-full max-w-6xl px-4 md:px-8 py-8">
        ...
        {/* Preset Picker & Edge Case Inspiration section */}
        {showPicker && (
          <div className={`w-full p-6 rounded-3xl shadow-xl mb-8 border-4 animate-in fade-in slide-in-from-top-4 duration-300 ${themeClasses.card} ${isDarkMode ? 'border-slate-800' : 'border-indigo-50'}`}>
            <h2 className={`text-xl font-bold mb-1 ${themeClasses.headerText}`}>Choose a subject to test:</h2>
            <div className="flex items-start gap-2 mb-4">
              <Info className="text-indigo-500 shrink-0 mt-0.5" size={16} />
              <p className={`text-sm ${themeClasses.subText}`}>
                <strong>Why diversity matters:</strong> Testing with different categories (like organic fruit vs. metal tools) helps us see if the AI relies on textures, colors, or specific geometric shapes. A robust AI should recognize objects regardless of their complexity!
              </p>
            </div>
            ...
            {/* Edge Case Examples Sub-section */}
            <div className={`mt-6 pt-6 border-t ${isDarkMode ? 'border-slate-800' : 'border-slate-100'}`}>
              ...
              <p className={`text-sm mb-6 ${themeClasses.subText}`}>These are real-world situations that often confuse computers. Can you recreate them with the sliders?</p>
              
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {EDGE_CASE_EXAMPLES.map((example) => (
                  <div key={example.id} className={`group relative rounded-2xl overflow-hidden border p-3 flex flex-col items-center text-center transition-all hover:shadow-md ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-slate-50 border-slate-200'}`}>
                    <div className="w-full aspect-square rounded-xl overflow-hidden mb-3">
                      <img src={example.url} alt={example.label} className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-500" />
                    </div>
                    <span className={`text-xs font-bold uppercase tracking-wider mb-1 ${isDarkMode ? 'text-slate-300' : 'text-slate-700'}`}>{example.label}</span>
                    <span className={`text-[10px] leading-tight ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`}>{example.description}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        {/* Footer / Lesson */}
        <footer className={`w-full p-8 rounded-[40px] shadow-2xl transition-colors duration-300 ${isDarkMode ? 'bg-slate-900 border border-slate-800' : 'bg-slate-900'} text-white`}>
